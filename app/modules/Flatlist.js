...
import { Platform, AppRegistry, StyleSheet, View, Text, FlatList, TouchableOpacity, 
    ScrollView, Modal, ActivityIndicator, AsyncStorage } from 'react-native';
...
export default class Flatlist extends React.Component {
    ...
    async componentWillMount () {
        return fetch('https://launchlibrary.net/1.3/launch')
        ...
        .then ((responseJson) => {
            ...
            // }, function () {
                try {
                    AsyncStorage.setItem('@Key:launches', responseJson.launches);
                 } catch (error) {
                   // Error saving data
                 }
            // });
    ...
    componentDidMount () {
        AsyncStorage.getItem('@Key:launches', (err, value) => this.setState({
            isLoading: false, 
    ...
    async showAgenciesAsync() {
        let url = 'https://launchlibrary.net/1.3/agency';
        ...
        return (
            <View style = {{ flex: 1, flexDirection: 'column', backgroundColor: 'black', padding: 10 }}> 
                <View style = { styles.body }>
                        <Text style = { styles.header }>The Rocket Family</Text>
                        <FlatList
